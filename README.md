...
**Важно!** Данные нужно хранить отдельно от кода, т.е. необходимо следить за тем, чтобы файлы с данными помещались исключительно в папку `data`. В самой папке данные должны быть разложены по папкам `source` и `processed`, а имена папок внесены в исключения в файл `.gitignore`.  
